...
const MaterialPo = () => {
    ...
    useEffect(() => {
        if (flatpickrRef.current) {
            flatpickr(flatpickrRef.current, {
                ...
                onChange: (selectedDates, dateStr) => setDateSelected(dateStr.split('T')[0]),
            });
        }
    ...
    const validate = values => {
        ...
        if (!values.locationId) errors.locationId = 'Field is required';
        if (!values.supplierId) errors.supplierId = 'Field is required';
        if (!dateSelected) errors.date = 'Field is required';
        ...
        if (values.materialPos.length === 0) {
            ...
            values.materialPos.forEach((materialItem, index) => {
                if (!materialItem.materialId) {
                    if (!errors.materialPos) errors.materialPos = [];
                    if (!errors.materialPos[index]) errors.materialPos[index] = {};
                                                        ...
                                                        <tbody className="bg-white divide-y divide-gray-200">
                                                            ...
                                                            {values.materialPos.map((materialItem, index) => (
                                                                ...
                                                                <tr key={index} className='bg-white dark:bg-slate-700 dark:text-white'>
                                                                    ...
                                                                    <td className="px-5  border-b border-gray-200 text-sm justify-center">
                                                                        <IoMdTrash type="button" size={20} title='Delete row'
